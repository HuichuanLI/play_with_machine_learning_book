...
def normalize(X, axis=-1, p=2):  # 正规化数据集 X
    lp_norm = np.atleast_1d(np.linalg.norm(X, p, axis))
    lp_norm[lp_norm == 0] = 1
    return X / np.expand_dims(lp_norm, axis)
def euclidean_distance(one_sample, X):  # 计算一个样本与数据集中所有样本的欧氏距离的平方
...
class Kmeans():
    ...
    def init_random_centroids(self, X):  # 从所有样本中随机选取self.k样本作为初始的聚类中心
...
def main():
    ...
    fig = plt.figure(figsize=(12, 8))  # 可视化聚类效果
